...
class Request
{
    ...
    private static $noRebuildUrl = false;
    ...
    // Custom function - do not rebuild url (required for EXPA's broken API)
    public static function rebuildUrl($status)
    {
        return self::$noRebuildUrl = !$status;
    }
    ...
    public static function send($method, $url, $body = null, $headers = array(), $username = null, $password = null)
    {
        ...
        if ($method !== Method::GET) {
            ...
            if (self::$noRebuildUrl)
            {
                $posturl = "";
                foreach ($body as $segment_name => $segment_body)
                {
                    if (substr($segment_name, 0, 1) == "+")
                        $posturl .= substr($segment_name, 2) . "=" . $segment_body;
                    else
                        $posturl .= "$segment_name=$segment_body";
                    $posturl .= "&";
                }
                $posturl = substr($posturl, 0, strlen($posturl)-1);
                $url .= urldecode($posturl);
            }
            else
        ...
        if (self::$debugMode)
        {
            echo "Calling ".(self::encodeUrl($url))."\n";
        }
